...
.. todo:: More module docstring (if needed. Too lazy to read code)
"""
...
class SHABP(ShabpOut):
    ...
    def read_shabp(self, shabp_filename, read_special_routines=False):
        ...
        while i < len(lines)-1:
            ...
            while 1:
                ...
                x1, y1, z1 = line[:10], line[10:20], line[20:30]
                t1 = line[30:31]  # Wait what's going on here?
				t1 = line[30]     # Wait what's going on here?
                x1 = float(x1)
                y1 = float(y1)
                z1 = float(z1)
                t1 = int(t1)
                if t1 == 1:
                    ...
                    row.append([x1, y1, z1])
                    patch.append(row)
					# msg = f'len(row)={len(row)} nrows={nrows} ncols={ncols}'
                    # assert len(row) == nrows, msg
                    break
                else:
                    raise RuntimeError()
                ...
                if t1 == 3:
                    break
                x2, y2, z2 = line[31:41], line[41:51], line[51:61]
				
				t2 = line[61:62]  # Wait what's going on here?
                t2 = line[61]     # Wait what's going on here?
                x2 = float(x2)
                y2 = float(y2)
                z2 = float(z2)
                t2 = int(t2)
    ...
    def get_points_elements_regions(self):
        ...
        for ipatch in range(npatches):
            ...
            for irow in range(nrows-1):
                for jcol in range(ncols-1):
                    element = [irow*ncols + jcol, (irow+1)*ncols + (jcol),
					           (irow+1)*ncols + (jcol+1), 
							   irow*ncols + (jcol+1),]
    ...
    def parse_trailer(self, read_special_routines=False):
        """Parses the case information
		
			e.g., number of angles of attack, control surface info
		"""
...
def parse_inviscid_pressure(lines, line, i):
    ...
    for icomponent in range(ncomponents):
        ...
        params[icomponent] = [impact, shadow, iprint, ipin, isave, pdata1, 
		                      pdata2, pdata3, pdata4, pdata5, pdata6]
        i += 1
